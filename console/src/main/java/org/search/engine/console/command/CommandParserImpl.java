    ...
    public Command parse(String input) throws CommandParseException {
        ...
        if (isIndex(inputList.get(0))) {
            ...
            if (inputList.size() != 2) {
                ...
                //Check for ) total number and if is in right index
                validateParenthesis(prefixParenthesisCounter, suffixParenthesisCounter, tokenCounter, inputList.subList(1, inputList.size()));
    ...
    List<String> splitInput(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (whitespace == tmpChar && tmpWord.length() > 0) {
                ...
                tmpWord = "";
            } else if (prefixParenthesis == tmpChar || suffixParenthesis == tmpChar || andOperation == tmpChar || orOperation == tmpChar) {
                if (tmpWord.length() > 0) {
                    wordList.add(tmpWord);
